    ...
    const renderMenuItems = (items: IMenuItem[]) => {
        return items.map((item) => {
            if (item.items && item.items.length > 0) {
                return (
                    <Menu.SubMenu key={item.id} icon={item.icon} title={<span className="text-m-medium">{item.label}</span>}>
    ...
    return (
        <ConfigProvider theme={theme}>
            <div className="flex h-screen overflow-hidden">
                ...
                >
                    <Sider className="h-full border-r flex flex-col" theme="light" width={300} collapsed={collapsed} collapsible trigger={null}>
                        ...
                        <div className="flex justify-end p-4 cursor-pointer" onClick={() => setCollapsed(!collapsed)}>
                ...
                <Layout className="flex-1 overflow-hidden">
                    ...
                    <Footer className="text-center text-gray-500">Study Hub © 2024 Created by Akalong</Footer>
                </Layout>
            </div>
        </ConfigProvider>
    );
